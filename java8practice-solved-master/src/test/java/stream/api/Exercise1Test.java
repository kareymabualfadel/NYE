...
public class Exercise1Test extends ClassicOnlineStore {
    ...
    public void findRichCustomers() {
        ...
        Stream<Customer> richCustomerStream =
                customerList.stream().filter(richCustomerCondition);

        assertTrue("Solution for Predicate should be lambda expression",
                AssertUtil.isLambda(richCustomerCondition));
    ...
    public void howOldAreTheCustomers() {
        ...
        Stream<Integer> ageStream =
                customerList.stream().map(getAgeFunction);
